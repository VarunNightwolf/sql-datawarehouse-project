...
from gold.fact_sales f left join gold.dim_customers c on c.customer_key = f.customer_key where order_date is not null), customer_aggregation as (
select customer_key, customer_number, customer_name, age, count(distinct order_number) as total_order,
...
else total_sales / total_order end as avg_order_value,
total_sales, total_quantity, total_products,
case when lifespan = 0 then total_sales
else total_sales / lifespan end as avg_monthly_spend,
last_order_date, lifespan from customer_aggregation
